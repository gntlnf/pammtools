...
# if(!is.null(temp$enum)) temp <- rename(temp, transition = enum)
...
# calculate the probability to go from 0 to every possible state

unique(test$trans_prob) ## verstehe nicht warum die ergebnisse unterschiedlich sind angeblich



for(i in 1:(max(temp$enum)+1)) {
